...
class CreateGashaponStoreSalesReportViewsTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    private $view_name = 'gashapon_store_sales_report';
    public function up(): void
    {
        ...
        DB::statement("
            ...
            SELECT
                ...
                CONCAT(
                    EXTRACT(
                        YEAR
                        FROM
                            `gashapon_store_sales`.`sales_date`
                    ),
                    '_',
                    LPAD(
                        EXTRACT(
                ...
                COALESCE(
                    `gashapon_store_sales`.`digits_code_rr_ref`,
                    `gashapon_store_sales`.`item_code`
                ) AS `digits_code_rr_ref`,
                `items`.`digits_code` AS `digits_code`,
                COALESCE(
                    `items`.`item_description`,
                    `gashapon_store_sales`.`item_description`
            ...
            FROM `gashapon_store_sales`
                LEFT JOIN `systems` ON `gashapon_store_sales`.`systems_id` = `systems`.`id`
                LEFT JOIN `organizations` ON `gashapon_store_sales`.`organizations_id` = `organizations`.`id`
                LEFT JOIN `report_types` ON `gashapon_store_sales`.`report_types_id` = `report_types`.`id`
                LEFT JOIN `channels` ON `gashapon_store_sales`.`channels_id` = `channels`.`id`
                LEFT JOIN `customers` ON `gashapon_store_sales`.`customers_id` = `customers`.`id`
                LEFT JOIN `concepts` ON `customers`.`concepts_id` = `concepts`.`id`
                LEFT JOIN `employees` ON `gashapon_store_sales`.`employees_id` = `employees`.`id`
                LEFT JOIN `apple_cutoffs` ON `gashapon_store_sales`.`sales_date` = `apple_cutoffs`.`sold_date`
                LEFT JOIN `non_apple_cutoffs` ON `gashapon_store_sales`.`sales_date` = `non_apple_cutoffs`.`sold_date`
                LEFT JOIN `items` ON `gashapon_store_sales`.`item_code` = `items`.`digits_code`;      
        ");
    }
