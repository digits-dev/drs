...
class SupplierIntransitInventoryImportJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $chunk = SupplierIntransitInventoryUploadLine::getWithHeader($this->chunk_id);
        ...
        SupplierIntransitInventory::upsert($insertable, ['batch_number', 'reference_number'], $columns);

        $count = SupplierIntransitInventory::where('batch_number',$chunk->batch)->count();

        if ($count == $chunk->row_count) {
            $supplies_intransit_inventory_upload = SupplierIntransitInventoryUpload::find($chunk->supp_intransit_inventory_uploads_id);
            $supplies_intransit_inventory_upload->update(['status' => 'IMPORT FINISHED']);
        }
    }

    public function failed($e) {
        $error_message = $e->getMessage();
        $chunk = SupplierIntransitInventoryUploadLines::getWithHeader($this->chunk_id);
        $supplies_intransit_inventory_upload = SupplierIntransitInventoryUpload::find($chunk->supp_intransit_inventory_uploads_id);
        $supplies_intransit_inventory_upload->update(['status' => 'IMPORT FAILED']);
        $supplies_intransit_inventory_upload->appendNewError($error_message);
    }
}
