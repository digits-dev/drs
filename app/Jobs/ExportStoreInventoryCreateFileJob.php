...
class ExportStoreInventoryCreateFileJob implements ShouldQueue
{
    ...
    public function handle(){
        ...
        $storeInventory = StoreInventory::filterForReport(StoreInventory::generateReport(), $this->filters)
            ->where('is_final', 1)
            ->take($this->chunkSize)
            ->get();
        ...
        (new \Rap2hpoutre\FastExcel\FastExcel($this->salesGenerator($storeInventory)))
            ->export(storage_path("app/{$this->folder}/ExportStoreInventory.csv"), function ($sale) {
                $salesReport = [];
                $salesReportCon = [];
