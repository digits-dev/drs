...
class GashaponStoreSalesImportJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $chunk = GashaponStoreSalesUploadLine::getWithHeader($this->chunk_id);
        ...
        GashaponStoreSales::upsert($insertable, ['batch_number', 'reference_number'], $columns);

        $count = GashaponStoreSales::where('batch_number', $chunk->batch)->count();
        Log::info("$count of $chunk->row_count rows imported for batch #$chunk->batch.");
        if ($count == $chunk->row_count) {
            $store_sales_upload = GashaponStoreSalesUpload::find($chunk->store_sales_uploads_id);
