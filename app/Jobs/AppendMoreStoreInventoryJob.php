...
class AppendMoreStoreInventoryJob implements ShouldQueue
{
    ...
    {
        $storeSales = StoreInventory::filterForReport(StoreInventory::generateReport(), $this->filters)
            ->where('is_final', 1)
        ...
        $file = storage_path("app/{$this->folder}/ExportStoreInventory.csv");
        $open = fopen($file, 'a+');
