...
class SupplierIntransitInventoryUpload extends Model
{
    use HasFactory;
    protected $guarded = [];

    public function getBatchDetails($id = null) {
        return $this->where('supplier_intransit_inventory_uploads.id', $id ?? $this->id)
            ->select(
            ...
            ->leftJoin('cms_users', 'cms_users.id', 'supplier_intransit_inventory_uploads.created_by')
            ->first();
    }
