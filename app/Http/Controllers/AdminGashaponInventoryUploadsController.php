<?php namespace App\Http\Controllers;
	...
	use App\Exports\GashaponInventoryUploadBatchExport;
	...
	class AdminGashaponInventoryUploadsController extends \crocodicstudio\crudbooster\controllers\CBController {
		public $status_class = [
	    ...
	    public function cbInit() {
			...
			$this->table = "gashapon_inventory_uploads";
	        ...
	        */
			...
			if (CRUDBooster::getCurrentMethod() == 'getIndex') {
					$this->index_button[] = [
						...
						"icon"=>"fa fa-upload",
						"color"=>"success",
						"url"=>route('gashapon-inventory.upload-view')
					];
			}
		...
		public function actionButtonSelected($id_selected,$button_name) {
			if ($button_name == 'tag_as_final') {
				foreach ($id_selected as $id) {
					$batch = GashaponInventoryUpload::find($id);
					...
					$supplier_intransit_inventory = GashaponInventory::where('batch_number', $batch->batch)->update(['is_final' => 1]);
				}
			...
			if ($button_name == 'tag_as_reject'){
                GashaponInventoryUpload::whereIn('id', $id_selected)
                ->update([
                    'status' => 'REJECTED',
	    ...
	    */
		...
		public function generateFile($id) {
			...
			$batch_export = new GashaponInventoryUploadBatchExport($batch->batch);
			$filename = $batch->batch. '.csv';
			$report_type = 'gashapon-inventory-upload';
		...
		public function getDetail($id) {
			...
			$gashapon_inventory_upload = (new GashaponInventoryUpload())->getBatchDetails($id);
			$user_report = ReportPrivilege::myReport(9, CRUDBooster::myPrivilegeId());
			$gashapon_inventories = GashaponInventory::filterForReport(GashaponInventory::generateReport(), ['search' => $search_term], true)
				->where('batch_number', $gashapon_inventory_upload->batch)
				->orderBy('reference_number', 'ASC')
				->paginate(10)
				->appends(['search' => $search_term]);

			$data = [];
			$data['item'] = $gashapon_inventory_upload;
			$data['user_report'] = $user_report;
			$data['gashapon_inventories'] = $gashapon_inventories;
			$data['search_term'] = $search_term;

			return $this->view('gashapon-inventory-upload.details', $data);
		}
