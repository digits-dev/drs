<?php namespace App\Http\Controllers;
	...
	use App\Exports\GashaponStoreSalesUploadBatchExport;
	use App\Jobs\UpdateBatchImportStatusJob;

	class AdminGashaponStoreSalesUploadsController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "gashapon_store_sales_uploads";
	    ...
	    public function actionButtonSelected($id_selected,$button_name) {
			if ($button_name == 'tag_as_final') {
				foreach ($id_selected as $id) {
					$batch = GashaponStoreSalesUpload::find($id);
					...
					$store_sales = GashaponStoreSales::where('batch_number', $batch->batch)->update(['is_final' => 1]);
				}
	    ...
	    */
		...
		public function generateFile($id) {
			...
			$batch_export = new GashaponStoreSalesUploadBatchExport($batch->batch);
			$filename = $batch->batch. '.csv';
			$report_type = 'gashapon-store-sales-upload';
		...
		public function getDetail($id) {
			...
			$gashapon_store_sale_upload = (new GashaponStoreSalesUpload())->getBatchDetails($id);
			$user_report = ReportPrivilege::myReport(1, CRUDBooster::myPrivilegeId());
			$gashapon_store_sales = GashaponStoreSales::filterForReport(GashaponStoreSales::generateReport(), ['search' => $search_term], true)
				->where('batch_number', $gashapon_store_sale_upload->batch)
				->orderBy('reference_number', 'ASC')
				->paginate(10)
				->appends(['search' => $search_term]);

			$data = [];
			$data['item'] = $gashapon_store_sale_upload;
			$data['user_report'] = $user_report;
			$data['gashapon_store_sales'] = $gashapon_store_sales;
			$data['search_term'] = $search_term;

			return $this->view('gashapon-store-sales-upload.details', $data);
		}
