...
class EtpStoreSyncReportController extends \crocodicstudio\crudbooster\controllers\CBController {
    ...
    public function getIndex(){
		...
		dd(Cache::get('StoreSyncDIGITAL WALKER.MEGAWORLD.FESTIVEWALK.RTL'));
		...
		$store_sales = DB::table('store_sales')->select('customer_name', DB::raw('MAX(sales_date) as latest_sales_date'))->leftJoin('customers', 'store_sales.customers_id', '=', 'customers.id')->groupBy('customer_name')->orderBy('latest_sales_date', 'desc')->get();
		$store_inventories = DB::table('store_inventories')->select('customer_name', DB::raw('MAX(inventory_date) as latest_inventory_date'))->leftJoin('customers', 'store_inventories.customers_id', '=', 'customers.id')->groupBy('customer_name')->orderBy('latest_inventory_date', 'desc')->get();
		...
		// dd($store_sales, $storesMap, $data['store_sync_data']);
