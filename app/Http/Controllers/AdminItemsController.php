	...
	class AdminItemsController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    */
		...
		public function importData() {
			...
			return view('import.digits-items-upload', $data)->render();
		}

		public function importItemsTemplate(){
			$filename = "digits-items"."-".date("Ymd").".csv";
		...
		public function importPostSave(Request $request) {
			...
			Excel::import(new DigitsItemsImport, $path);	
			CRUDBooster::redirect(CRUDBooster::adminpath('items'), trans("Upload Successfully!"), 'success');
		}

		public function getDimfsData(){
			$api = config('link-api.get-created-items-url');
			$ch = curl_init($api);
			...
			foreach ($all_items as $item) {
				...
				$to_be_inserted['inventory_type_description']  = $item['inventory_type_code'];
