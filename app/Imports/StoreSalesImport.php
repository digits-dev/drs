    ...
    WithBatchInserts
{
    ...
    public function __construct($batch_number)
    {
        ...
        $this->report_type = ReportType::byName("STORE SALES");
    }
    ...
    */
    ...
    {
        // $soldDate = Carbon::parse($this->transformDate($row["sold_date"]))->format("Y-m-d");
        ...
        // $v_apple_cutoff = $this->apple_cutoff->where('sold_date',$soldDate)->first();
        // $v_non_apple_cutoff = $this->non_apple_cutoff->where('sold_date',$soldDate)->first();
        ...
        StoreSale::updateOrCreate([
            ...
            'pos_transaction_type'  => $row['pos_transaction_type'] ?? NULL
            ...
            // 'non_apple_week_cutoff' => $v_non_apple_cutoff->non_apple_week_cutoff,
            // 'sales_date_yr_mo'      => Carbon::parse($this->transformDate($row["sold_date"]))->format("Y_m"),
            // 'sales_year'            => Carbon::parse($this->transformDate($row["sold_date"]))->format("Y"),
            // 'sales_month'           => Carbon::parse($this->transformDate($row["sold_date"]))->format("m"),
        ]);
    }
    ...
    public static function afterImport(AfterImport $event)
    {
        ...
        $config['to'] = CRUDBooster::adminPath('store_sales');
